...
func main() {
	...
	println("start", os.Args[1])
	ScanLimit, _ := strconv.Atoi(os.Args[1])
	...
	for _, xfile := range xfiles {
		...
		slog.Println(slog.DEBUG, "file", xfile)

		if In_array(xfile, scanDF) {
			slog.Println(slog.DEBUG, "扫过file", xfile)
			continue
		}
		...
		for line, data := range datas {
			...
			if line < lineLast {
				slog.Println(slog.DEBUG, "扫过line", line)
				continue
			}
...
func Read(path string) string {
	...
	if err != nil {
		slog.Println(slog.DEBUG, "router.Run error", err)
	}
